...
<div class="title-wrapper mb-6">
  <div class="flex justify-between items-center">
    <div>
      ...
      <p class="text-sm">Quản lý sinh viên và phân công giảng viên hướng dẫn đợt thực tập #<?= htmlspecialchars($batch['id']) ?></p>
    </div>
  ...
  <div class="detail-layout__main">
    ...
    <div class="card shadow-sm">
      <div class="tm-container" data-tm="batch_students_table" data-tm-mode="client" data-tm-searchable="true"
        ...
        <!-- Cột Lớp -->
        <template data-tm-col="classroom_name" data-tm-label="Lớp" data-tm-filter-type="text" data-tm-sortable data-tm-width="120px">
          <span class="text-sm font-semibold">{{ value || '--' }}</span>
        </template>

        <!-- Cột Công ty thực tập -->
        <template data-tm-col="company_name" data-tm-label="Công ty thực tập" data-tm-filter-type="text" data-tm-sortable data-tm-width="250px">
        ...
        <template data-tm-col="teacher_name" data-tm-label="Giảng viên HD" data-tm-sortable data-tm-width="280px">
          <div class="teacher-cell" data-assignment-id="{{ row.assignment_id || '' }}" data-batch-student-id="{{ row.batch_student_id }}" data-teacher-id="{{ row.teacher_id || '' }}">
  ...
  <aside class="detail-layout__sidebar">
    <div class="card shadow-sm h-full flex flex-col">
      <div class="card__header flex justify-between items-center">
        <h3 class="font-bold text-lg"><i class="fa-solid fa-chalkboard-user mr-2"></i>Giảng viên <span class="badge" data-variant="primary" id="supervisor-count">0</span></h3>
      </div>
      <hr class="separator">
...
<div id="modal-bulk-assign" class="modal-overlay hidden" data-state="closed">
  <div class="modal-content shadow-xl">
    <div class="modal-header">
      ...
      <p class="text-sm mt-1">Gán <span id="bulk-student-count" class="font-bold">0</span> sinh viên cho giảng viên dưới đây:</p>
    </div>
  ...
  <div class="modal-content shadow-xl">
    ...
    <div class="modal-body mb-6">
      ...
      <ul class="flex flex-col gap-2 text-sm">
        <li class="flex gap-2"><i class="fa-solid fa-check text-success mt-1"></i> Tìm tất cả sinh viên chưa có giảng viên hướng dẫn.</li>
        <li class="flex gap-2"><i class="fa-solid fa-check text-success mt-1"></i> Ưu tiên phân công cho giảng viên đang hướng dẫn ÍT SINH VIÊN NHẤT.</li>
        <li class="flex gap-2"><i class="fa-solid fa-check text-success mt-1"></i> Đảm bảo không vượt quá hạn mức của từng giảng viên.</li>
      </ul>
    </div>
    <div class="modal-footer flex justify-end gap-2">
      <button type="button" id="btn-close-even-modal" class="btn" data-size="md" data-variant="outline">Để sau</button>
      <button type="button" id="btn-confirm-auto-even" class="btn" data-size="md" data-variant="primary">Tiến hành ngay</button>
    </div>
  </div>
</div>
...
<div id="modal-auto-shuffle" class="modal-overlay hidden" data-state="closed">
  <div class="modal-content shadow-xl">
    ...
    <div class="modal-body mb-6">
      ...
      <ul class="flex flex-col gap-2 text-sm">
        <li class="flex gap-2"><i class="fa-solid fa-check text-success mt-1"></i> Tìm tất cả sinh viên chưa có giảng viên hướng dẫn.</li>
        <li class="flex gap-2"><i class="fa-solid fa-check text-success mt-1"></i> Phân công ngẫu nhiên cho các giảng viên còn hạn mức trống.</li>
        <li class="flex gap-2"><i class="fa-solid fa-check text-success mt-1"></i> Đảm bảo không vượt quá hạn mức của từng giảng viên.</li>
      </ul>
    </div>
    <div class="modal-footer flex justify-end gap-2">
      <button type="button" id="btn-close-shuffle-modal" class="btn" data-size="md" data-variant="outline">Để sau</button>
      <button type="button" id="btn-confirm-auto-shuffle" class="btn" data-size="md" data-variant="primary">Tiến hành ngay</button>
    </div>
  </div>
</div>
