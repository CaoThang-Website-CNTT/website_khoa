...
<h2 class="title-wrapper__title">Công ty nghi trùng lặp</h2>
...
<div class="flex flex-col gap-4">
  <?php if (empty($groups)): ?>
    <div class="card p-2 text-center">
      Không tìm thấy công ty nào nghi trùng lặp.
    </div>
  <?php else: ?>
      ...
      <div class="card p-0">
        ...
        <div class="company-group-header">
          ...
          <div class="company-group-header__actions">
            ...
            ?>
            <form id="form-bulk-merge-<?= $parent['id'] ?>" action="<?= url('admin/companies/bulk-quick-merge') ?>" method="POST" class="m-0 inline-block">
              <?= csrf_field() ?>
              <?php foreach ($childIds as $cid): ?>
                <input type="hidden" name="source_ids[]" value="<?= $cid ?>">
              <?php endforeach; ?>
              <input type="hidden" name="target_id" value="<?= $parent['id'] ?>">
              <button type="button" class="btn btn-confirm-trigger" data-variant="primary" data-size="md" data-form-id="form-bulk-merge-<?= $parent['id'] ?>" data-message="Bạn có chắc chắn muốn gộp TẤT CẢ các công ty dưới đây vào công ty gốc (#<?= $parent['id'] ?>)? Toàn bộ sinh viên và giấy giới thiệu thực tập sẽ được tự động chuyển sang công ty gốc, sau đó các công ty bị gộp sẽ bị xóa khỏi hệ thống. Lưu ý: Hành động này không thể hoàn tác.">
        ...
        <!-- Body: Children Companies -->
        <div class="p-2">
          <div class="font-semibold">Danh sách công ty nghi ngờ trùng lặp (Sẽ bị xóa nếu gộp):</div>
                ...
                <?php foreach ($group['children'] as $child): ?>
                  <tr>
                    ...
                    <td class="text-sm p-2">
                      ...
                      <span class="badge mt-1" data-variant="<?= $child['is_verified'] ? 'primary' : 'warning' ?>" data-size="sm">
                    ...
                    <td class="text-sm p-2 company-duplicates-table__td--action">
                      <form id="form-quick-merge-<?= $child['id'] ?>" action="<?= url('admin/companies/quick-merge') ?>" method="POST" class="m-0 inline-block">
                        <?= csrf_field() ?>
                        <input type="hidden" name="source_id" value="<?= $child['id'] ?>">
                        <input type="hidden" name="target_id" value="<?= $parent['id'] ?>">
                        <button type="button" class="btn btn-confirm-trigger" data-variant="secondary" data-size="sm" title="Gộp nhanh và xóa công ty này" data-form-id="form-quick-merge-<?= $child['id'] ?>" data-message="Bạn có chắc muốn gộp nhanh công ty #<?= $child['id'] ?> vào công ty gốc #<?= $parent['id'] ?>? Toàn bộ sinh viên và giấy giới thiệu thực tập sẽ được tự động chuyển sang công ty gốc, sau đó công ty #<?= $child['id'] ?> sẽ bị xóa. Lưu ý: Hành động này không thể hoàn tác.">
                          <i class="fa-solid fa-bolt"></i> Gộp nhanh
                        </button>
                      </form>
                      <a href="<?= url("admin/companies/{$child['id']}/merge?q=" . urlencode($parent['name'])) ?>" class="btn" data-variant="outline" data-size="sm" title="Mở trang gộp thủ công">Gộp thủ công
                      </a>
                    </td>
                  </tr>
                <?php endforeach; ?>
...
<div class="modal" id="confirm-modal" tabindex="-1" data-state="closed">
  <div class="modal__header">
    ...
    <p class="modal__description" id="confirm-modal-message">
      Bạn có chắc chắn muốn thực hiện thao tác này? Toàn bộ sinh viên và giấy giới thiệu thực tập của các công ty bị gộp sẽ được tự động chuyển sang công ty gốc, sau đó các công ty bị gộp sẽ bị xóa khỏi hệ thống. Lưu ý: Hành động này không thể hoàn tác.
    </p>
  </div>
    ...
    document.querySelectorAll('.btn-confirm-trigger').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (typeof ModalHandler !== 'undefined' && ModalHandler.instance) {
          ...
          console.error('ModalHandler is not defined');
        }
      });
    });
