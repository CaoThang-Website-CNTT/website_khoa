...
<form id="merge-form" action="<?= url('admin/companies/' . $company->id . '/merge') ?>" method="POST">
  ...
  <div id="mst-warning" class="company-merge-warning hidden">
    <div class="flex gap-3">
      ...
      <div>
        ...
        <p class="text-sm mt-1">Hai công ty này có mã số thuế khác nhau. Việc gộp chỉ nên thực hiện khi bạn chắc chắn đây là cùng 1 công ty (VD: công ty đổi MST, nhập sai MST).</p>
  ...
  <div class="card p-0">
    <div class="company-merge-layout">
      ...
      <div class="company-merge-layout__target">
        <div class="relative">
          ...
          <div class="w-full">
            <input type="text" id="target_search" class="field__input" placeholder="Tìm theo tên hoặc MST..." autocomplete="off">
          ...
          <div id="target_selected_info" class="mt-2 hidden">
            ...
            <button type="button" id="btn_clear_target" class="company-merge-search__clear" title="Chọn lại"><i class="fa-solid fa-xmark"></i></button>
          </div>
        </div>
      </div>
    </div>
      ...
      ?>
        <div class="merge-row field-row" data-field="<?= $key ?>">
          ...
          <div class="merge-cell merge-cell--center">
            ...
            <input type="hidden" name="<?= $key ?>" id="<?= $key ?>_input" value="<?= htmlspecialchars($company->$key ?? '') ?>">
          </div>
        </div>
      <?php endforeach; ?>
    </div>
    ...
    <div class="company-merge-impact">
      ...
      <p class="company-merge-impact__note">Tất cả sẽ được chuyển sang tham chiếu đến công ty đích sau khi gộp thành công.</p>
    </div>
  </div>
