  ...
  <main>
    ...
    <div id="be-topbar">
      <div id="be-topbar-left">
        ...
        <button type="button" class="btn" data-size="md" data-variant="outline" id="be-toggle-left" title="Ẩn/hiện panel block">
    ...
    <div id="be-body">
      <div class="be-panel__wrapper">
        ...
        <div id="be-left" class="be-panel" data-tabs data-tabs-id="be-left-panel" data-tabs-panel-active="be-blocks-panel" data-tabs-sync="false">
          <div class="tabs__list be-panel__tabs-list" role="tablist">
            <button type="button" class="be-panel__tabs-trigger be-left-tab active" data-tabs-trigger="be-blocks-panel">Blocks</button>
            <button type="button" class="be-panel__tabs-trigger be-left-tab" data-tabs-trigger="be-list-view-panel">Cấu trúc</button>
          </div>
          ...
          <div class="be-canvas__content-wrapper">
            <div class="be-canvas__content be-canvas-title-area">
              ...
              <div class="be-canvas-meta">
                <div class="be-canvas-meta__info" data-be-meta-preview="settings.show_author" data-be-preview-action="toggle">
                ...
                <div class="be-canvas-meta__info" data-be-meta-preview="settings.show_date" data-be-preview-action="toggle">
                  <i class="fa-regular fa-calendar"></i> <span class="be-canvas-meta__text"><?= date('d/m/Y') ?></span>
                </div>
                <div class="be-canvas-meta__info" data-be-meta-preview="settings.show_view_count" data-be-preview-action="toggle">
                  <i class="fa-regular fa-eye"></i>
                  <span class="be-canvas-meta__text">
                    <span class="be-canvas-meta__value" data-be-meta-preview="init_view_count" data-be-preview-action="text" data-preview-default="0">0</span> lượt xem
                  </span>
                </div>
              </div>
            </div>
      ...
      <div class="be-panel__wrapper">
        <div class="be-panel__gap"></div>
        <div id="be-right" class="be-panel" data-tabs data-tabs-id="be-right-panel" data-tabs-panel-active="be-post-settings-panel" data-tabs-sync="false">
          <div class="tabs__list be-panel__tabs-list" role="tablist">
            <button type="button" class="be-panel__tabs-trigger be-right-tab" data-tabs-trigger="be-post-settings-panel">Bài viết</button>
            <button type="button" class="be-panel__tabs-trigger be-right-tab active" data-tabs-trigger="be-block-settings-panel">Block</button>
          </div>
          <div class="be-panel__content">
            ...
            <div class="tabs__panel" data-tabs-panel="be-post-settings-panel" role="tabpanel" id="be-post-settings-panel">
              <div class="field-group">
                ...
                <div class="field">
                  ...
                  <button type="button" class="select" data-select-id="be-author-select" data-select-searchable data-select-placeholder="Chọn tác giả" name="author_id" data-be-meta-key="author_id" role="listbox"
                    data-select-default-value="<?= $initialPayload['meta']['author_id'] ?? $current_user['account_id'] ?>">
                    <div class="select__content">
                      <?php foreach (($authors ?? []) as $author): ?>
                        <div class="select__item" data-select-value="<?= $author->id ?>"><?= htmlspecialchars($author->email) ?></div>
                      <?php endforeach; ?>
                    </div>
                  </button>
                </div>
                ...
                <div class="field">
                  ...
                  <button type="button" class="select" data-select-id="be-status-select" name="status" data-be-meta-key="status" role="listbox" data-select-default-value="<?= $initialPayload['meta']['status'] ?? 'draft' ?>">
                    <div class="select__content">
                ...
                <div class="field">
                  ...
                  <input type="text" class="field__input" id="be-slug-input" name="slug" value="<?= htmlspecialchars($initialPayload['meta']['slug'] ?? '') ?>" placeholder="duong-dan-bai-viet" data-be-meta-key="slug">
                </div>

                <div class="field">
                  <span class="field__label">Danh mục</span>
                  <button type="button" class="select" data-select-id="be-categories-select" data-select-multiple data-select-searchable data-select-placeholder="Chọn danh mục..." name="category_ids" data-be-meta-key="category_ids" role="listbox" <?= empty($categories) ? 'data-select-disabled' : '' ?>
                    <?php $selectedCats = $initialPayload['meta']['category_ids'] ?? []; if (!empty($selectedCats)): ?> data-select-default-value="<?= implode(',', array_map('intval', $selectedCats)) ?>" <?php endif; ?>>
                    <div class="select__content">
                      <?php if (!empty($categories)): foreach ($categories as $cat): ?>
                        <div class="select__item" data-select-value="<?= $cat->id ?>" <?= ($cat->disabled ?? false) ? 'data-select-disabled' : '' ?>><?= htmlspecialchars($cat->name) ?></div>
                      <?php endforeach; else: ?>
                ...
                <div class="field">
                  ...
                  <textarea id="be-excerpt-input" class="field__input" rows="3" name="seo_desc" placeholder="Để trống: tự lấy từ đoạn văn đầu tiên" maxlength="500" data-be-meta-key="excerpt"><?= htmlspecialchars($initialPayload['meta']['excerpt'] ?? '') ?></textarea>
                </div>
    ...
    <form id="be-post-form" method="POST" action="<?= $isEdit ? url("admin/posts/{$post->id}") : url("admin/posts"); ?>" class="hidden">
      <?= csrf_field() ?>
      <?php if ($isEdit): ?>
