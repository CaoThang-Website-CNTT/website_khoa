...
export class CmsPreview {
  ...
  markActiveEditable() {
    ...
    if (active.sectionId) doc.querySelector(`[data-section-id="${cssEscape(active.sectionId)}"]`)?.classList.add('is-active');
  }
  ...
  #bindFrame() {
    ...
    doc.addEventListener('click', (event) => {
      ...
      const icon = event.target.closest('[data-cms-icon-edit="true"]'); const section = event.target.closest('[data-section-id]');
      if (editable) this.bus.dispatch('preview:editable_selected', { sectionId: editable.dataset.sectionId, path: editable.dataset.cmsPath, value: plainEditableText(editable) });
      else if (image) this.bus.dispatch('preview:image_selected', { sectionId: image.dataset.sectionId, path: image.dataset.cmsPath });
