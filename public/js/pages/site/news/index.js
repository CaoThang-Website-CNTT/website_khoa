document.addEventListener('DOMContentLoaded', function () {
  ...
  const pageSubtitle = document.querySelector('.section-title__subtitle');
  const allNewsTitle = document.getElementById('all-news-title');
  ...
  const defaultPageTitle = 'Tin tức & Sự kiện';
  const defaultPageSubtitle = 'Cập nhật thông tin mới nhất từ Khoa CNTT';
  const defaultDocumentTitle = document.title;
  const categoryLabels = Array.from(filterBtns).reduce(function (labels, button) {
    labels[button.dataset.category || 'all'] = button.textContent.trim();
    return labels;
  }, {});
  ...
  function categoryFromLocation() {
    ...
    const queryCategory = params.get('category') || params.get('filter');
    if (queryCategory) return queryCategory;

    const currentPath = normalizePath(window.location.pathname);
    const urlMap = config.urlMap || {};

    for (const [category, mappedUrlStr] of Object.entries(urlMap)) {
      try {
        const mappedUrl = new URL(mappedUrlStr, window.location.origin);
        if (normalizePath(mappedUrl.pathname) === currentPath) {
          return category;
        }
      } catch (e) {
        if (normalizePath(mappedUrlStr) === currentPath) {
          return category;
        }
      }
    }
  ...
  function activeTitle() {
    if (state.search) return 'Kết quả tìm kiếm tin tức';
    if (state.category !== 'all') return categoryLabels[state.category] || defaultPageTitle;
    return defaultPageTitle;
  }

  function syncPageText(total = null) {
    const isFiltered = Boolean(state.search || state.category !== 'all');
    const title = activeTitle();

    if (pageTitle) pageTitle.textContent = title;
    if (allNewsTitle) allNewsTitle.textContent = isFiltered ? 'Kết quả' : 'Tất cả tin tức';
    if (pageSubtitle) {
      pageSubtitle.textContent = isFiltered && total !== null
        ? `Tìm thấy ${AppUtils.formatNumber(total)} bài viết phù hợp`
        : defaultPageSubtitle;
    }
    document.title = title === defaultPageTitle ? defaultDocumentTitle : title;
  }
  ...
  async function requestPosts(queryState, overrides = {}) {
    ...
    return payload?.data && typeof payload.data === 'object' && Array.isArray(payload.data.data)
      ? payload.data
      : payload;
  }
  ...
  async function fetchPosts(options = {}) {
    ...
    try {
      ...
      syncPageText(payload.meta?.total ?? payload.total ?? posts.length);
      syncLoadMore();
    } catch (error) {
