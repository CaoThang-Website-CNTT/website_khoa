...
document.addEventListener("DOMContentLoaded", () => {
  const tableContainer = document.querySelector(
    '[data-tm="referral_letters_table"]',
  );
  ...
  if (tableContainer) {
    tableContainer.addEventListener("tm:state-change", async (e) => {
      ...
      const url = new URL(
        `${window.API_BASE_URL}/internship/batches/${window.BATCH_ID}/management/referral-letters`,
      );
    ...
    TableManager.registerBulkActions("referral_letters_table", {
      ...
      actions: [
...

        {
          ...
          onClick: ({ selectedIds }) => {
            currentSelectedIds = selectedIds;
            document.getElementById("cancel-count").textContent =
  ...
  const callBulkActionApi = async (action, reason = "") => {
    try {
      TableManager.setBulkActionLoading(
        "referral_letters_table",
        true,
        "Đang xử lý...",
      );

      const res = await fetch(
        `${window.API_BASE_URL}/internship/batches/${window.BATCH_ID}/management/referral-letters/bulk-action`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          apiErrorMessage(
            result,
            `Không thể ${verb} các giấy giới thiệu đã chọn. Vui lòng kiểm tra trạng thái của từng giấy.`,
          ),
        );
      }
  ...
  document
    ...
    ?.addEventListener("click", () => {
      const reason = document
        .getElementById("cancel_reason_input")
        .value.trim();
      if (!reason) {
        if (window.toast) {
  ...
  document
    .getElementById("btn-confirm-receive")
    ?.addEventListener("click", async () => {
      const payload = {
        recipient_name: document.getElementById("recipient_name").value.trim(),
        recipient_phone: document
          .getElementById("recipient_phone")
          .value.trim(),
        recipient_email: document
          .getElementById("recipient_email")
          .value.trim(),
      };
      try {
        const response = await fetch(
          `${window.API_BASE_URL}/internship/batches/${window.BATCH_ID}/management/referral-letters/${currentSelectedIds[0]}/receive`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!response.ok)
          throw new Error(
            apiErrorMessage(
              result,
              "Không thể xác nhận nhận giấy. Giấy phải ở trạng thái Hoàn thành và thông tin người nhận phải hợp lệ.",
            ),
          );
        window.toast?.success(
          "Thành công",
          result.message || "Đã xác nhận nhận giấy.",
        );
