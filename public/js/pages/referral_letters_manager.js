  ...
  if (tableContainer) {
    tableContainer.addEventListener('tm:state-change', async (e) => {
      ...
      if (state.sort?.column) {
        url.searchParams.set('sort', `${state.sort.column}:${state.sort.direction}`);
      }

      if (state.filters && state.filters.length > 0) {
        state.filters.forEach((filter, index) => {
          url.searchParams.set(`filters[${index}][col]`, filter.column);
          url.searchParams.set(`filters[${index}][op]`, filter.operator);
